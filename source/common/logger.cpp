...
namespace LM_INFER_ENGINE {
    namespace COMMON {
        ...
        LmInferLogger& LmInferLogger::Instance()
        {
            static LmInferLogger log;
            return log;
        }

        void LmInferLogger::initLogger(std::string file_name, int log_level)
        {
            // set log level
        ...
        void LmInferLogger::setLevel(int level)
        {
            // update log level
            if (nullptr != m_logger.get())
            {
